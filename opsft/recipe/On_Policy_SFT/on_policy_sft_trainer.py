...
class RayOnPolicySFTTrainer(RayPPOTrainer):
    """
    RayOnPolicySFTTrainer is a trainer for on-policy supervised fine-tuning (On-Policy SFT) using Ray.
    """
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.trainer.enable_train_reward and self.reward_fn is not None:
                        ...
                        # ✅ Filter data: Drop wrong answers and keep ALL correct responses.
                        batch = self._data_filter(batch)
                    ...
                    # Whether inference temperature is considered during training.
    ...
    def _data_filter(self, batch: DataProto) -> DataProto:
            """
            Filters samples by correctness for each query (uid):
            1. Keep ALL correct responses (score > 0).
            2. Drop the entire query if all responses are wrong.
            3. Truncate the final batch size to be divisible by GPU count (dp_size).
            """

            # 1. Get UIDs, total scores, and lengths
            uids = [str(u) for u in batch.non_tensor_batch["uid"]]
            scores = batch.batch["token_level_scores"].sum(-1)      
            resp_len = batch.batch["response_mask"].sum(-1) # Note: length is extracted but not used       

            # 2. Group samples by UID
            ...
            # 3. Filter correct responses
            for uid, lst in per_uid.items():
                # Find all correct answers (score > 0)
                correct = [(idx, s, l) for (idx, s, l) in lst if s > 0]
                
                # Drop the whole query if everything is wrong
                if len(correct) == 0:
                    continue
                
                # Keep ALL correct answers
                keep_indices.extend(idx for (idx, _, _) in correct)

            if not keep_indices:
                return batch.select_idxs([])

            # 4. Data Parallel (DP) alignment
            dp_size = getattr(self.actor_rollout_wg, "world_size", 1)
            keep_indices = sorted(set(keep_indices))

            # Truncate to make total size divisible by dp_size
            if dp_size > 1:
